...
export default function CuccheKeeper() {
  ...
  useEffect(() => {
    if (players.length > 0) {
      localStorage.setItem("cucche-game", JSON.stringify({ players, winner }));
    }
  }, [players, winner]);
    ...
    <div className="p-4 max-w-4xl mx-auto">
      {!gameStarted ? (
        <div className="space-y-4">
          <h1 className="text-2xl font-bold text-center">Start Chuche Game</h1>
          {names.map((n, i) => (
            <input
              key={i}
              type="text"
        ...
        <div className="space-y-6">
          ...
          {/* Save Normal Round Button on Top */}
          {!winner && (
            <button
              onClick={handleNormalSave}
              className="w-full bg-blue-600 text-white px-4 py-3 rounded text-lg mb-2"
            >
              Save Normal Round
            </button>
          )}

          {/* Score Table with Normal Round Inputs on Top */}
